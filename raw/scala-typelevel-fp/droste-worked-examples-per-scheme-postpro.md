# Droste Worked Example: `postpro` — `modules/tests/src/test/scala-2.13+/higherkindness/droste/tests/SmallPost.scala`

> Source: https://github.com/higherkindness/droste (main branch, `modules/tests/src/test/scala-2.13+/higherkindness/droste/tests/SmallPost.scala`)
...
Mirrors `SmallPre` (prepro): here `infiniteCoalg` unfolds an infinite lazy stream of increasing integers, and `filterNT` (a natural transformation `StreamF[Int, *] ~> StreamF[Int, *]`) truncates the stream to `EmptyF` past `lim`, applied *after* each unfold step — this is `postpro`'s "postprocess the output structure" behavior, the mirror image of `prepro`'s preprocessing before a fold.
