# Droste Source: `kernel.scala` + `scheme.scala` (hylo, cata, ana, ghylo/gcata/gana)

> Source: https://github.com/higherkindness/droste (main branch, `modules/core/src/main/scala/higherkindness/droste/kernel.scala` and `modules/core/src/main/scala/higherkindness/droste/scheme.scala`)
> Collected: 2026-09-12
> Published: Unknown

## `kernel.scala` — the two primitive fixed-point-free implementations everything else derives from
...
`kernel.hylo` is the single primitive every scheme in droste ultimately compiles to: it takes an `A`, unfolds it one step with `coalgebra: A => F[A]`, maps the recursive continuation (`this`) over the resulting `F[A]`, then folds the resulting `F[B]` with `algebra: F[B] => B`. No fixed-point type (`Fix`, `Mu`, `Nu`) is required — this is what makes `hylo` able to run without ever materializing an intermediate recursive structure.

## `scheme.scala` — the public API: `ana`, `cata`, `hylo`, and generalized `ghylo`/`gcata`/`gana`
...
Notes on the `scheme` API surface: `scheme.cata`/`scheme.ana`/`scheme.hylo` (plus their `M`-suffixed monadic variants) live directly on `scheme` because they're the ones beginners reach for first. Everything generalized (`ghylo`, `gcata`, `gana`, and their `M` variants) takes a `GAlgebra.Gathered`/`GCoalgebra.Scattered` pair — i.e., an algebra/coalgebra plus a `Gather`/`Scatter` strategy — and is what the named "zoo" schemes (`para`, `zygo`, `histo`, `apo`, ...) are defined in terms of. `scheme.zoo` is a separate nested object holding the more exotic/less common named schemes so they don't clutter `scheme`'s own namespace for newcomers. `scheme.apply[PatR[_[_]]]` is a convenience entry point (`scheme[Fix]`, `scheme[MyCofreeShape]`, etc.) that fixes the recursive "shape" up front so Scala's type inference doesn't need every type parameter spelled out at each call site.
